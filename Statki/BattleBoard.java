    ...
    public void printBothBoards(BattleBoard opponent){
      ...
      public void shipAdd(){
        ...
        for (int i=0; i <shipLengths.length; i++){
            System.out.println("To jest twoja plansza:");
            printBoard();
            System.out.println("Wpisz, gdzie chcesz skierować statek (długość " + shipLengths[i] + ")");
            String input = scan.nextLine();
            String[] check = input.split(" ");
            int[] coords = stringToCoord(check[0]);
            boolean valid = checkValid(check[1], shipLengths[i], coords);
            while(!valid){
                System.out.println("Nieprawidłowe umieszczenie statku");
                System.out.println("Wpisz, gdzie chcesz skierować statek (długość " + shipLengths[i] + ")");
                input = scan.nextLine();
                check = input.split(" ");
